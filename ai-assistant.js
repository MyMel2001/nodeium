...
function initAIAssistant() {
    // Load all settings from localStorage or global settings
    loadAISettings();

    // Initialize settings UI (now handled by settings.js)
...
/**
 * Load AI settings from localStorage or global settings.
 */
function loadAISettings() {
    // Try to use global settings first (from settings.js)
    if (window.aiSettings) {
        apiKey = window.aiSettings.apiKey;
        apiUrl = window.aiSettings.apiUrl;
        AI_CONFIG.model = window.aiSettings.model;
        AI_CONFIG.temperature = window.aiSettings.temperature;
        AI_CONFIG.maxTokens = window.aiSettings.maxTokens;
        AI_CONFIG.contextAware = window.aiSettings.contextAware;
    } else {
        ...
        AI_CONFIG.maxTokens = parseInt(localStorage.getItem('ai-max-tokens') || '1000', 10);
...
function initializeSettingsUI() {
    // Settings UI is now handled by settings.js
 ...
 * @deprecated Settings are now in the main settings modal
 */
function toggleAISettings() {
    // Open the main settings modal instead
 ...
 * @deprecated Settings are now saved by settings.js
 */
function saveAISettings() {
    // Settings are now saved by settings.js
    // This function is kept for compatibility
}

/**
 * Reset AI Settings to defaults.
 * @deprecated Settings are now reset by settings.js
 */
function resetAISettings() {
    // Settings are now reset by settings.js
    // This function is kept for compatibility
}
...
// Helper functions for settings.js to update AI settings
window.setAIApiKey = function(key) {
    apiKey = key;
};

window.setAIApiUrl = function(url) {
    apiUrl = url;
};

// **NOTE:** You must ensure the following functions are defined elsewhere in your Nodeium application:
// go(), back(), forward(), refresh()
// The HTML elements must also exist: aiSidebar, chatInput, chatMessages, etc.
